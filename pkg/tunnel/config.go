...
import (
	...
	"github.com/isannai/mesh/pkg/stationwire"
)
...
func LoadConfig(path string) (Config, error) {
	...
	// Station auto-wire: when this config lives under the apps/ tree (a station
	// config), the effective services = engines derived from apps/ (zero-config)
	// with the file's `services` applied as overrides. Non-apps configs (broker,
	// tests) resolve root="" → no derivation, services stay as loaded.
	if root := stationwire.RootFromConfigPath(path); root != "" {
		if merged, derr := stationwire.ResolveServices(root, cfg.Services); derr == nil {
			cfg.Services = merged
		}
	}
