...
// ExtractArchive extracts a release archive to destDir, dispatching by file
// extension: .tar.gz/.tgz → ExtractTarGz (Linux packages, exec bit preserved),
// anything else → ExtractZip (Windows packages). Callers that download an asset
// whose format follows ivm.AssetName() (per-OS) use this so one call handles
// both formats.
func ExtractArchive(path, destDir string) ([]ExtractedFile, error) {
